...
int write_file(ptr_t grid, const char *file_name, const file_helper_t* h, MPI_Comm active_procs);
...
int main(int argc, char **argv) {
    ...
    double start, end, pre_time, comm_time, calc_time;
#ifdef ALIGNED
    ptr_t __attribute__((aligned(64))) a0, __attribute__((aligned(64))) a1;
    ptr_t __attribute__((aligned(64))) ans0, __attribute__((aligned(64))) ans1;
#else
    ...
    MPI_Comm_rank(MPI_COMM_WORLD, &info.p_id);// 进程号
    MPI_Comm_size(MPI_COMM_WORLD, &info.p_num);// 进程总数
    ABORT_IF_ERROR(parse_args(&info, &nt, &type, argc, argv))// 处理命令行参数
    ...
    a0 = memory_alloc(&info);// 实际分配的内存是local_size + 2*halo
    ...
    /* warm up */
    calc_time = comm_time = 0.0;
    if (type == 7) {
        ans0 = stencil_7(a0, a1, &info, 1, &calc_time, &comm_time);
    } else {
        ans0 = stencil_27(a0, a1, &info, 1, &calc_time, &comm_time);
    }
    ...
    start = MPI_Wtime();
    if (type == 7) {
        ans0 = stencil_7(a0, a1, &info, nt, &calc_time, &comm_time);
    } else {
        ans0 = stencil_27(a0, a1, &info, nt, &calc_time, &comm_time);
    }
    MPI_Barrier(MPI_COMM_WORLD); 
    end = MPI_Wtime();
    ...
    if(status == 0) {
        if(argc == 8) {
            ...
            if (info.p_id == 0) {
                ...
                if(result.norm_inf > 1e-9) {
                    printf("pid: %d    Significant numeric error.\n", info.p_id);
                }
            }
        } else if(p_id == 0) { 
        ...
        if(p_id == 0) { 
            ...
            printf("Total time %lfs, Calc time %lfs, Comm time %lfs, Performance %lfGflop/s\n", t, calc_time, comm_time, gflops);
        }
...
int find_active_procs(const dist_grid_info_t *info, MPI_Comm *active_procs) {
    ...
    MPI_Comm_split(MPI_COMM_WORLD, status, info->p_id, active_procs);//将status相同的进程放在一个通信域
    return status;
}
...
int write_file(ptr_t grid, const char *file_name, const file_helper_t* h, MPI_Comm active_procs) {
    ...
    MPI_Status status;
    int count;
    int ret = MPI_File_open(active_procs, file_name, 5, MPI_INFO_NULL, &input);
    CHECK_ERROR(ret, ret)
    ret = MPI_File_set_view(input, 0, DATA_TYPE, h->file_view, "native", MPI_INFO_NULL);
    CHECK_ERROR(ret, ret)
    ret = MPI_File_write_all(input, grid, 1, h->array_center, &status);
    //ret = MPI_File_read_all(input, grid, 1, h->array_center, &status);
    CHECK_ERROR(ret, ret)
    ret = MPI_File_close(&input);
    CHECK_ERROR(ret, ret)
...
ptr_t memory_alloc(const dist_grid_info_t *info) {
#ifdef ALIGNED
    ptr_t __attribute__((aligned(64))) tmp;
#else
    ptr_t tmp;
#endif    
